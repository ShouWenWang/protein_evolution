function [Atom_type,coords]=read_PDB_data_C_beta(PDB_name,reference_index,cut_index_low,cut_index_high)

%% Atom_type: -3 is C-alpha atom; 0 is C-beta atom
% if an amino acid has both Calpha and Cbeta atoms, it is pattern is -3 0,
% where C alpha comes first. 
...
relevantAtom=zeros(size(modelstruct.Atom(:),1),2);
relevantAtom(:,2)=5;  % 5 is the state that is not either Calpha or Cbeta

for i=1:size(modelstruct.Atom(:),1) %loop over atoms
    atom_name_temp=char(modelstruct.Atom(i).AtomName);
    if length(atom_name_temp)==2  && char(modelstruct.Atom(i).chainID)==char('A')
        if  atom_name_temp(1)==char('C') &&  atom_name_temp(2)==char('A') 
        relevantAtom(i,1)=1;relevantAtom(i,2)=-3; % C alpha

        
        else if  atom_name_temp(1)==char('C') &&  atom_name_temp(2)==char('B')
        relevantAtom(i,1)=1;relevantAtom(i,2)=0; % C beta
...
relevantAtom(1:low_index-1,2)=0;
relevantAtom(high_index+1:end,2)=0;
...
count=0;
Atom_type=zeros(1,1); % size could increase dynamically
for i=1:size(modelstruct.Atom(:),1)
    if relevantAtom(i,1)==1 
        count=count+1;
        Atom_type(count)=relevantAtom(i,2);
    end
